...
void clear_screen() {
    system("cls");  // Gunakan "clear" untuk Linux/Mac
}
...
komik* partition(komik* head, komik* end, komik** newHead, komik** newEnd) {
    ...
    while (current != pivot) {
        if (current->tahunTerbit >= pivot->tahunTerbit) {  // Sorting descending
...
komik* quick_sort_recursive(komik* head, komik* end) {
    if (!head || head == end) return head;  // Kondisi dasar
...
void quick_sort_tahun(komik** head_ref) {
    komik* end = *head_ref;
    while (end && end->next) end = end->next;  // Dapatkan elemen terakhir
